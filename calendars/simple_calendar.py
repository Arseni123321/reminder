...
class SimpleCalendar:
    ...
    async def start_calendar(self, year: int = datetime.now().year,
        ...
        """последняя строка - кнопки предыдущего месяца"""
        inline_kb.row()
        inline_kb.insert(InlineKeyboardButton(
            '<',
    ...
    async def process_selection(self, query: CallbackQuery, data: CallbackData):
        """Обработка callback_query.Этот метод создает новый календарь при нажатии кноки вперед и назад.
        Этот метод нужно вызывать внутри CallbackQueryHandler

        :param query: callback_query, kak eto
        :param data: callback_data, slovar sosdan s pomoshu calendar_callback
        :return: Кортедж с датой
        """
        ...
        """Обработка нажатия на кнопку следущего года"""
        if data['action'] == 'NEXT_YEAR':
        ...
        """Обработка нажатия на кнопку ghtlsleotuj года"""
        if data['action'] == 'PREV_YEAR':
