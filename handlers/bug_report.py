...
async def send_report(message: types.Message, state: FSMContext):
    send_report_answer = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text='Ответить', callback_data=bug_report_callback_info.new(user_id=message.from_user.id))
    )
    await bot.copy_message(config.tg_bot.ADMIN_ID, message.from_user.id, message.message_id,
                           reply_markup=send_report_answer)
    await message.reply('Спасибо за отчет', reply_markup=types.ReplyKeyboardRemove())
    await state.finish()
